...
class AshbySource(OpportunitySource):
    source_name = "ashby"
    ...
    def collect(self, organization: OrganizationConfig) -> list[SourceRecord]:
        url = f"https://api.ashbyhq.com/posting-api/job-board/{organization.identifier}"
        try:
            payload = get_json(self.session, url, self.settings.collection_timeout_seconds)
        except requests.RequestException as exc:
            logger.warning(
                "Ashby collection failed",
        ...
        if not isinstance(jobs, list):
            return []
        return [SourceRecord(source=self.source_name, organization=organization, raw=job) for job in jobs]
